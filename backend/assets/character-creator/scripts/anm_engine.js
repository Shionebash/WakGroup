...
class AnmEngine {
  ...
  render(animName, timeMs, scale, cx, cy, opts = {}) {
    ...
    const targetWidth = this.canvas.width * 0.88;
    const targetHeight = this.canvas.height * 0.90;
    ...
    const offsetY = cy - bounds.maxY * fittedScale;
   ...
   */
  _computeStableBounds(animName, spriteDef, animSourceLayer, flipH) {
    const cacheKey = animName + '|flip=' + (flipH ? '1' : '0') + '|' + this.layers.map(l => l.id).join(',');
